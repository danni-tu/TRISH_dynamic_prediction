...
```{r read_in_data, cache = TRUE, eval = F}
...
plot_fun_oos <- function(dir_dat = day1.path, covar_type = "full", outcome = "LRM_50"){
  ...
  out_snip = switch(outcome, "LRM_50" = "l", "LRM_Z" = "lzr")
...
```{r eval = TRUE}
# Performance, by t
plot_fun_oos(covar_type = "full", outcome = "LRM_Z")[[1]] %>% grid.draw()# Performance averaged over t
plot_fun_oos(covar_type = "full", outcome = "LRM_Z")[[3]] %>% pt()
# Rank table
rank_table_fun_oos(covar_type = "full", outcome = "LRM_Z")
```
...
```{r eval = TRUE}
# Performance, by t
plot_fun_oos(covar_type = "top10", outcome = "LRM_Z")[[1]] %>% grid.draw()# Performance averaged over t
plot_fun_oos(covar_type = "top10", outcome = "LRM_Z")[[3]] %>% pt()
# Rank table
rank_table_fun_oos(covar_type = "top10", outcome = "LRM_Z")
```

# Appendix: Helper Functions

This was saved as `helpers.R` in the `code.path` directory and sourced at the beginning of this code.
...
# dat = RST data
# n_days = split data so that the testing set has the last n_days per participant,
# and the training set has all the preceding data
divide_data_day <- function(dat = rst.smooth, n_days = 15){
  ## Prediction
  ## Form training and test sets by splitting data in half by person
  rst.split <- dat %>%
    filter(Test_Track_Type != "Other") %>%
    # For each participant and test type, split data in two
    group_by(Subject_ID) %>%
    arrange(abs_days_sub) %>%
    mutate(max_day = max(abs_days_sub),
           Test = (abs_days_sub > (max_day-n_days))) %>%
    split(.$Test) %>%
    purrr::map(ungroup) %>%
...
# Similar to divide_data_day, but uses # observations instead of the number of days
divide_data_obs_subj <- function(dat_subj, n_train_obs = NULL, n_test_obs){
  n_subj = nrow(dat_subj)
  
  # If n_train_obs not provided, just use all data available
  if (is.null(n_train_obs)){
    dat_subj2 <- dat_subj %>%
      mutate(obs2 = 1:n(),
             Test = (obs2 %in% tail(x = obs2, n = n_test_obs)))
    
    return(dat_subj2)
  } 
  
  # If there isn't enough data to train on n_train_obs days
  # just use the days available
  if ((n_train_obs + n_test_obs) > n_subj){
    dat_subj2 <- dat_subj %>%
      mutate(obs2 = 1:n(),
             Test = (obs2 %in% tail(x = obs2, n = n_test_obs)))
  } else {
    # Otherwise, split data into the first n_train_obs observations
    # And the following n_test_obs observations
    total_obs = n_train_obs + n_test_obs
    
    dat_subj2 <- dat_subj %>%
      mutate(obs2 = 1:n()) %>%
      filter(obs2 <= total_obs) %>%
      mutate(Test = (obs2 > n_train_obs)) 
  }
...
divide_data_obs <- function(dat = rst.smooth, n_train_obs, n_test_obs = 1){
  
  # If # training obs not provided, use all data minus testing data
  if (missing(n_train_obs)){
    rst.split <- dat %>%
      filter(Test_Track_Type != "Other") %>%
      # For each participant and test type, split data in two
      split(.$Subject_ID) %>%
      map(divide_data_obs_subj, n_test_obs = n_test_obs) %>%
      Reduce(rbind,.)
  } else {
    rst.split <- dat %>%
      filter(Test_Track_Type != "Other") %>%
      # For each participant and test type, split data in two
      split(.$Subject_ID) %>%
      map(divide_data_obs_subj, n_train_obs = n_train_obs, n_test_obs = n_test_obs) %>%
      Reduce(rbind,.)
  }
...
# (code from https://github.com/tidyverse/ggplot2/issues/4088)
...
fcr_heatmap_1var <- function(dat0 = rst.smooth,
  ...
  dat_fake <- dat_fake0 %>%
    # First average over all participants/Male/Test Track Type
    group_by(Subject_ID, subj, Male, Test_Track_Type) %>%
    summarize_all(mean, na.rm = TRUE) %>%
    ungroup %>%
    # Then take average over all participants
